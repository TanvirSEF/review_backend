...
# Application startup-e tables automatic generate korbe (Alembic-er dorkor hobe na shortcut validation task-e)
...
# Next.js frontend theke request block bypass korar jonne CORS middleware configuration
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# 1. Get all products with Average Rating
@app.get("/api/products", response_model=List[schemas.ProductListResponse])
def get_all_products(db: Session = Depends(get_db)):
    # SQL query block using modern SQLAlchemy 2.0 select syntax
    # Left outer join perform kore aggregate function দিয়ে average_rating layer design
    query = (
        select(
            models.Product.id,
...
# 2. Get Single Product Details with All Embedded Reviews
...
def get_product_details(id: int, db: Session = Depends(get_db)):
    # Product search engine check query
    ...
    # Reviews load mapping matching product id
    ...
    # Validation scheme formatting directly binding user names recursively
    formatted_reviews = []
    for r in reviews:
        # User details fetching for review array metadata injection
...
def create_product_review(review: schemas.ReviewCreate, db: Session = Depends(get_db)):
    # Product validity check pipeline step
    ...
    # User validity layer validation check step
    ...
    # Instance generation logic execution mapping
...
def update_product_review(id: int, review_update: schemas.ReviewUpdate, db: Session = Depends(get_db)):
    ...
    # Field mapping assignment loop overrides
